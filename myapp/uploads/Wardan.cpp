...
int main()
{
	char ch;
	stack s;								  //Stack
	string infix = "(3 + 2) + 7/2 * ((7+3)*2)"; //Infix expression
	...
	for (int i = 0; i < infix.length(); i++)
	{
		...
		{
			int p = precedence(ch);
			while (!s.isEmpty() && precedence(s.stackTop()) >= p)
			{
				postfix += s.stackTop();
				s.pop();
			}
...
// 		Convert infix to postfix Infix : 8 *
// 		(3 + 4) / 2 -
// 	3 * 5 Postfix : 834 + *2 / 35 * -Evaluating postfix expression Postfix expression : 834 + *2 / 3
